test_that("report data prepares readable sections from an audit", {
  ...
  data <- safebiome:::audit_report_data(audit)
  ...
  expect_s3_class(data$plan, "safebiome_analysis_plan")
...
test_that("report validates inputs and locates its template", {
  expect_error(report(list()), "safebiome_audit")
  expect_error(report(safebiome:::biome_audit(), file = NA_character_), "file")
  expect_error(report(safebiome:::biome_audit(), quiet = NA), "quiet")
  expect_true(file.exists(safebiome:::safebiome_report_template()))
})
...
test_that("report passes audit data to the R Markdown renderer", {
  ...
  testthat::local_mocked_bindings(
    ...
    render_report_rmarkdown = function(input, output_file, output_dir, params, envir, quiet, ...) {
      ...
      writeLines("<html>safebiome audit report</html>", rendered)
      rendered
    }
  )
  ...
  expect_s3_class(captured$params$audit, "safebiome_audit")
...
test_that("report dependency checks are explicit", {
  ...
  expect_error(safebiome:::check_report_dependencies(), "rmarkdown")

  testthat::local_mocked_bindings(
    report_has_rmarkdown = function() TRUE,
    report_has_pandoc = function() FALSE
  )
  expect_error(safebiome:::check_report_dependencies(), "Pandoc")

  testthat::local_mocked_bindings(
    report_has_rmarkdown = function() TRUE,
    report_has_pandoc = function() TRUE
  )
  expect_invisible(safebiome:::check_report_dependencies())
})
...
test_that("render_report_rmarkdown delegates to the renderer implementation", {
  ...
  expect_equal(
    safebiome:::render_report_rmarkdown(input = "template.Rmd", quiet = TRUE),
    "rendered.html"
  )
...
test_that("report helpers cover fallback audit sections", {
  ...
  risk <- safebiome:::audit_report_risk(audit)
  batch <- safebiome:::audit_report_batch(audit)
  design <- safebiome:::audit_report_data_frame(audit$design)

  expect_equal(risk$module, "overall")
  expect_equal(batch$status, "skipped")
  expect_equal(design$status, "skipped")
  expect_equal(safebiome:::report_value(NULL), "Not supplied")
  expect_equal(safebiome:::report_value(character()), "Not supplied")
  expect_equal(safebiome:::report_value(c("a", "b")), "a, b")
})
