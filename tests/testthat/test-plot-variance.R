test_that("plot_variance returns a ggplot for one distance", {
  ...
  plot_data <- safebiome:::permanova_terms_plot_data(audit, "bray")
...
test_that("plot_variance supports all audited distances", {
  ...
  plot_data <- safebiome:::permanova_terms_plot_data(audit, c("aitchison", "bray"))
...
test_that("plot_variance defaults to the first available distance", {
  ...
  selected <- safebiome:::resolve_variance_distances(audit, distance = NULL)
